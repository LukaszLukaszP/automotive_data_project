...
PROCESSED_IDS = load_processed_ids()          ### Global set of processed IDs
ID_LOCK = threading.Lock()                    ### Lock for thread-safe ID updates
...
def split_link(url, session):
    ...
    and, if necessary, by gearbox type to avoid exceeding the 500-page limit.
    ...
    for start_year in range(min_year, max_year + 1):
        ...
        # Create a new link filtered by year range
...
def get_offer_links_from_page(url, session, processed_ids):
    """
    Extracts new offer IDs and URLs from a listing page.
    """
    result = []
    soup = get_soup(url, session)
    if not soup:
...
# -------------------------------------------------------------------
# Function to extract information from a single offer page
# -------------------------------------------------------------------
def extract_offer_data(soup):
    ...
    # ----------------------------------------------------------------
    # 1. BASIC INFORMATION (old layout first, then fallback new layout)
    # ----------------------------------------------------------------
    extracted_any = False
    ...
    if not extracted_any:
        SKIP_TESTIDS = {"basic_information", "technical_specs", "condition_history", 
...
# -------------------------------------------------------------------
# Process each split link: scrape listings, extract data, save CSV & IDs
# -------------------------------------------------------------------
def process_splitted_link(idx: int, url: str, session: requests.Session) -> int:
    ...
    for page_num in range(1, pages_to_scrape + 1):
        ...
        for advert_id, offer_url in offers:
            ...
            # Insert advert_id into extracted data so CSV will record it
            extracted["advert_id"] = advert_id
